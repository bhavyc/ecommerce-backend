  ...
  async (req, res) => {
    try {
      const { 
        ...
        bankAccountNo, ifscCode, termsAccepted, indianGoods
      } = req.body;

      if (!termsAccepted || !indianGoods) {
        return res.status(400).send("You must accept Terms & Conditions.");
      }
      ...
      const profile = new SellerProfile({
        seller: user._id,
        businessName, businessType, panNumber, gstNumber,
        address, city, state, pincode, bankAccountNo, ifscCode,
        termsAccepted: true, indianGoods: true, status: "under_review"
      });
      await profile.save();

      // ✅ CHANGE 3: Save Cloudinary URLs
      // Ab 'req.files.xxx[0].path' mein direct HTTPS link milega
