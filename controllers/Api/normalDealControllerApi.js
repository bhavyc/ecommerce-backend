...
exports.getAllDeals = async (req, res) => {
  try {
    ...
    const totalDocs = await NormalDeal.countDocuments(filter);

    const deals = await NormalDeal.find(filter)
    ...
    const processedDeals = deals.map(deal => {
      ...
      return {
        ...deal.toObject(),
        price: deal.price,
        discount: forcedDiscount, // Frontend ko 0 milega
        finalPrice: deal.price,   // Final price = Original Price
      };
    });
    ...
    res.status(500).json({ success: false, message: "Error loading deals" });
  }
};